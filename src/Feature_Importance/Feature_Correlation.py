...
def get_correlated_features(features_df, corr_threshold):
    ...
    for feature_1 in corr_df.columns.tolist():
        a = corr_df[feature_1] > corr_threshold
...
def get_correlated_csv(filename, corr_threshold):
    ...
    df = reduce_memory_footprint(filename)
    # Remove bar columns for removing correlated features
    features_df = df.drop(['Bar'], axis=1)
    correlated_features = get_correlated_features(features_df, corr_threshold)
    ...
    return reduced_correlated_features

if __name__ == "main":
    get_correlated_csv('Full_Features.csv', 0.95)
